...
export async function PATCH(request: NextRequest, context: { params: Promise<{ site: string; id: string }> }) {
  const { site, id } = await context.params;
  const token = request.cookies.get('mouhibhub-auth')?.value ?? null;
  if (!token || !verifyAuthToken(token)) {
    return unauthorized();
  ...
  const result = await db.collection('images').findOneAndUpdate(filter, { $set: update }, { returnDocument: 'after' });
  if (!result.value) {
    return new NextResponse(JSON.stringify({ message: 'Image not found.' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
  }
